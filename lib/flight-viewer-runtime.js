...
function formatMetarWindCompact(metar) {
  const windSpeed = Number.isFinite(metar?.wind_speed_kt) ? Math.round(metar.wind_speed_kt) : null
  const windDirection = Number.isFinite(metar?.wind_dir_deg)
    ? Math.round(metar.wind_dir_deg)
    : null
  ...
  if ((windSpeed ?? 0) <= 1 && windDirection == null) {
    return "Calm"
  }

  if (windDirection == null) {
    return `VRB ${windSpeed ?? 0}kt`
  }

  if (windSpeed == null) {
    return `${String(windDirection).padStart(3, "0")}°`
  }

  return `${String(windDirection).padStart(3, "0")}° ${windSpeed}kt`
}

function formatMetarAltimeterCompact(metar) {
  return Number.isFinite(metar?.altimeter_in_hg) ? metar.altimeter_in_hg.toFixed(2) : "--"
}

function formatMetarVisibilityCompact(metar) {
  const visibility = String(metar?.visibility_sm || "").trim()
  if (!visibility) {
    return "--"
  }

  return /sm$/i.test(visibility) ? visibility.toUpperCase() : `${visibility}SM`
}

function getMetarPrimaryCloudTag(metar) {
  const cloudTag = Array.isArray(metar?.clouds)
    ? metar.clouds.map((layer) => formatCloudLayerTag(layer)).find(Boolean)
    : null
  if (cloudTag) {
    return cloudTag
  }
...
function getMetarConditionSummary(metar) {
  const weatherText = String(metar?.weather_text || "")
    .trim()
    .toUpperCase()

  if (weatherText.includes("TS")) {
    return { emoji: "⛈️", text: "Storm" }
  }

  if (/(SN|SG|PL|IP|IC)/.test(weatherText)) {
    return { emoji: "❄️", text: "Snow" }
  }

  if (/(RA|DZ|SH|UP)/.test(weatherText)) {
    if (weatherText.includes("DZ")) {
      return { emoji: "🌦️", text: "Drizzle" }
    }

    return { emoji: "🌧️", text: weatherText.includes("SH") ? "Showers" : "Rain" }
  }

  if (/(FG|BR|HZ|FU|VA)/.test(weatherText)) {
    if (weatherText.includes("FG")) {
      return { emoji: "🌫️", text: "Fog" }
    }

    return { emoji: "🌫️", text: weatherText.includes("BR") ? "Mist" : "Haze" }
  }

  if (Number.isFinite(metar?.precip_in) && metar.precip_in > 0) {
    return {
      emoji: "🌧️",
      text: `${metar.precip_in.toFixed(2)}in`
    }
  }

  const cloudTag = getMetarPrimaryCloudTag(metar)
  if (cloudTag === "Clear") {
    return { emoji: "☀️", text: "Clear" }
  }

  if (cloudTag && cloudTag !== "--") {
    return { emoji: "☁️", text: cloudTag }
  }

  return { emoji: "❔", text: "--" }
}

function getFlightCategoryAccent(category) {
...
function getSectorWeatherSummary() {
  const metars = state.airports
    .map((airport) => airport?.metar)
    .filter(Boolean)

  if (!metars.length) {
    return {
      category: state.lastUpdatedMs == null ? "WAITING" : "NO METAR",
      wind: "--",
      altimeter: "-- inHg",
      visibility: "--",
      condition: { emoji: "❔", text: state.lastUpdatedMs == null ? "Waiting" : "--" },
      clouds: "--"
    }
  }

  const categoryRank = { LIFR: 0, IFR: 1, MVFR: 2, VFR: 3 }
  const category = metars
    .map((metar) => String(metar.flight_category || "").toUpperCase())
    .filter(Boolean)
    .sort((left, right) => (categoryRank[left] ?? 99) - (categoryRank[right] ?? 99))[0] || "METAR"

  const averageWindSpeed = averageFiniteNumbers(metars.map((metar) => metar.wind_speed_kt))
  const averageWindDirection = averageFiniteNumbers(metars.map((metar) => metar.wind_dir_deg))
  const averageAltimeter = averageFiniteNumbers(metars.map((metar) => metar.altimeter_in_hg))
  const averagePrecip = averageFiniteNumbers(metars.map((metar) => metar.precip_in))
  const averageVisibility = averageFiniteNumbers(
    metars.map((metar) => Number.parseFloat(String(metar.visibility_sm || "").replace(/[^\d./]/g, "")))
  )

  const representativeMetar =
    metars.find((metar) => String(metar.weather_text || "").trim()) ||
    metars.find((metar) => Array.isArray(metar.clouds) && metar.clouds.length) ||
    metars[0]

  const wind =
    averageWindSpeed == null && averageWindDirection == null
      ? "--"
      : averageWindDirection == null
        ? `VRB ${Math.round(averageWindSpeed ?? 0)}kt`
        : `${String(Math.round(averageWindDirection)).padStart(3, "0")}° ${Math.round(averageWindSpeed ?? 0)}kt`

  const altimeter = averageAltimeter == null ? "-- inHg" : `${averageAltimeter.toFixed(2)} inHg`
  const visibility =
    averageVisibility == null ? "--" : `${averageVisibility < 10 ? averageVisibility.toFixed(1) : Math.round(averageVisibility)}SM`
  const condition =
    averagePrecip != null && averagePrecip > 0
      ? { emoji: "🌧️", text: `${averagePrecip.toFixed(2)}in` }
      : getMetarConditionSummary(representativeMetar)
  const clouds = getMetarPrimaryCloudTag(representativeMetar)
...
function getCompassCardLayout() {
  const cardWidth = 148
  const cardHeight = 166
  const cardX = state.width - cardWidth - 24
  const cardY = 20
...
function drawWeatherCards() {
  const compassLayout = getCompassCardLayout()
  const boxGap = 10
  const boxWidth = 116
  const boxHeight = 60
  const columns = 4
  const rows = 1
  const stackWidth = columns * boxWidth + (columns - 1) * boxGap
  const stackHeight = rows * boxHeight + (rows - 1) * boxGap
  const stackX = Math.max(16, compassLayout.cardX - 16 - stackWidth)
  const stackY = compassLayout.cardY
  const summary = getSectorWeatherSummary()
  const boxes = [
    {
      emoji: "🛩️",
      label: "Rules",
      value: summary.category,
      accent: getFlightCategoryAccent(summary.category)
    },
    {
      emoji: "💨",
      label: "Wind",
  ...
  boxes.forEach((box, index) => {
    ...
    const cardX = stackX + column * (boxWidth + boxGap)
    const cardY = stackY + row * (boxHeight + boxGap)
    ...
    ctx.beginPath()
    ctx.roundRect(cardX, cardY, boxWidth, boxHeight, 14)
    ctx.fill()

    ctx.shadowBlur = 0
    ctx.shadowOffsetY = 0
    ctx.strokeStyle = "rgba(142, 227, 255, 0.26)"
    ctx.lineWidth = 1
    ctx.stroke()

    ctx.fillStyle = "rgba(158, 183, 198, 0.86)"
    ctx.font = "600 10px 'Trebuchet MS', sans-serif"
    ctx.fillText(`${box.emoji} ${box.label}`, cardX + 12, cardY + 16)

    ctx.fillStyle = box.accent
    ctx.font = "700 13px 'Trebuchet MS', sans-serif"
    ctx.fillText(box.value, cardX + 12, cardY + 38)
  })
...
function drawCompass(basis) {
  const { cardWidth, cardHeight, cardX, cardY, centerX, centerY, radius } =
    getCompassCardLayout()
  ...
  const tickDirections = [
    ...
    {
      direction: getScreenDirection({ x: 1, y: 0, z: -1 }, basis),
      inner: radius - 5,
      outer: radius - 1,
      width: 1,
      color: "rgba(158, 183, 198, 0.44)"
    },
    {
      direction: getScreenDirection({ x: 1, y: 0, z: 1 }, basis),
      inner: radius - 5,
  ...
  const headingChipWidth = 88
  const headingChipHeight = 30
  const headingChipX = centerX - headingChipWidth * 0.5
  const headingChipY = cardY + 146
  ...
  ctx.strokeStyle = "rgba(142, 227, 255, 0.18)"
  ctx.lineWidth = 1
  ctx.stroke()

  ctx.fillStyle = "rgba(85, 214, 255, 0.08)"
  ctx.beginPath()
  ...
  tickDirections.forEach((tick) => {
    ctx.strokeStyle = tick.color
    ctx.lineWidth = tick.width
    ctx.beginPath()
    ctx.moveTo(
      centerX + tick.direction.x * tick.inner,
      centerY + tick.direction.y * tick.inner
    )
    ctx.lineTo(
      centerX + tick.direction.x * tick.outer,
      centerY + tick.direction.y * tick.outer
    )
    ctx.stroke()
  })

  ctx.strokeStyle = "rgba(85, 214, 255, 0.3)"
  ctx.lineWidth = 3.4
  ctx.beginPath()
  ctx.moveTo(centerX - north.x * 7, centerY - north.y * 7)
  ctx.lineTo(centerX + north.x * (radius - 8), centerY + north.y * (radius - 8))
  ctx.stroke()

  ctx.strokeStyle = "rgba(239, 247, 251, 0.96)"
  ctx.lineWidth = 1.8
  ctx.beginPath()
  ctx.moveTo(centerX - north.x * 7, centerY - north.y * 7)
  ctx.lineTo(centerX + north.x * (radius - 8), centerY + north.y * (radius - 8))
  ...
  const leftX = centerX + north.x * 9 - north.y * 5
  const leftY = centerY + north.y * 9 + north.x * 5
  const rightX = centerX + north.x * 9 + north.y * 5
  const rightY = centerY + north.y * 9 - north.x * 5
  ...
  ctx.textBaseline = "middle"
  ctx.fillStyle = "#55d6ff"
  ctx.fillText("N", centerX + north.x * (radius + 17), centerY + north.y * (radius + 17))
  ctx.fillStyle = "#d7e7ef"
  ctx.fillText("E", centerX + east.x * (radius + 16), centerY + east.y * (radius + 16))
  ctx.fillText("S", centerX + south.x * (radius + 16), centerY + south.y * (radius + 16))
  ctx.fillText("W", centerX + west.x * (radius + 16), centerY + west.y * (radius + 16))

  ctx.fillStyle = "rgba(239, 247, 251, 0.12)"
  ctx.beginPath()
  ctx.roundRect(headingChipX, headingChipY, headingChipWidth, headingChipHeight, 14)
  ctx.fill()
  ...
  ctx.fillText("HEADING", centerX, headingChipY - 11)
  ctx.fillStyle = "#eff7fb"
  ctx.font = "700 15px 'Trebuchet MS', sans-serif"
  ctx.fillText(
    formatCompassHeadingDegrees(headingDegrees),
    centerX,
    headingChipY + headingChipHeight * 0.5 + 1
  )

  ctx.fillStyle = "rgba(239, 247, 251, 0.9)"
  ctx.beginPath()
